<div class="row m-0 p-0 justify-content-center">
    <div class="col-lg-8 col-xl-6 m-0 p-0 py-5">
        ...
        <hr>
        <h5>Fokus Riset Dosen</h5>
        <p>Fokus riset tidak tersedia? <a href="{{ url('admin-fokus-riset') }}" class="text-clr1 td-hover">Tambah di sini.</a></p>
        <hr>
        <form action="{{ url('admin-simpan-dosen/step-3') }}" method="post">
            ...
            <?php $i = 1 ?>
            @foreach($dosen->fr_list as $x)
                <div class="mb-3 d-flex align-items-center">
                    <input type="checkbox" name="fr_id[]" value="{{ $x->fr_id }}" id="fr_id-{{ $x->fr_id }}" class="cursor-pointer" {{ ($x->fr_selected == true) ? 'checked' : '' }}>
