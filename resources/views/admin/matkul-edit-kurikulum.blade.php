@php
    ...
    if(session()->has('session_kurikulum')) {
        $kurikulum = session('session_kurikulum');
        $selectedExists = !empty(array_filter($kurikulum, fn($item) => $item['selected'] === true));
    }
@endphp
<div id="containerKurikulum">
    @if(session()->has('session_kurikulum') && $selectedExists)
    ...
    @elseif($mk->kurikulum->isNotEmpty())
        @foreach($mk->kurikulum as $x)
            <div class="my-2 px-3 lh-1 d-flex align-items-center @if($x->kurikulum_selected == false) d-none @endif">
                <i class="fas fa-check-circle me-3"></i>
                <div>
                    <p class="m-0">{{ $x->kurikulum_judul_ID }}</p>
                    <p class="text-secondary fsz-10 m-0">{{ $x->kurikulum_judul_EN }}</p>
