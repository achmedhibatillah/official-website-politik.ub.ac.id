<div class="text-clr3 row m-0 p-0 w-100">
    <div class="d-flex align-items-top m-0 p-0">
        <div class="m-0 p-0 mt-4 mt-lg-0 flex-grow-1">
            <h3 class="lh-1">{{ $berita->berita_judul_ID }}</h3>
            <p class="text-secondary lh-1 ms-1">{{ $berita->berita_judul_EN }}</p>
            @include('templates/session')
            <hr>
            <div class="d-flex lh-1 mb-2">
                <p class="m-0 fsz-11">Url:</p>
                <a href="{{ url('berita/' . $berita->berita_slug) }}" class="td-hover fsz-11 ms-2 text-clr1">lihat halaman di halaman pengunjung <i class="fa-solid fa-arrow-up-right-from-square fsz-10"></i></a>
            </div>
            <div class="d-flex">
                <p class="text-clr3 fsz-11 border-clr3 lh-1 py-1 px-2">https://politik.ub.ac.id/berita/{{ $berita->berita_slug }}</p>
            </div>
            <div class="mt-3">
            </div>
        </div>
        <div class="ms-3 mb-3 border-clr4 shadow-m p-3 d-flex align-items-center m-0">
            <div class="">
            <div class="d-flex justify-content-center align-items-center overflow-hidden rounded shadow-l-2" style="width:100px;aspect-ratio:3/2;">
                <img src="{{ asset($berita->berita_gambar) }}" style="height:100%;width:100%;object-fit:cover;">
            </div>
            <a href="#" data-bs-toggle="modal" data-bs-target="#modalImage" class="text-center fsz-10 text-clr1 td-hover">Perbesar gambar</a>
            </div>
        </div>
    </div>
    <hr>
    <div class="m-0 p-0">
        <table class="fsz-11">
            <tr>
                <td>Dibuat</td>
                <td>:</td>
                <td class="ps-2 font-price fw-light fsz-10 text-secondary">{{ $berita->created_at }}</td>
            </tr>
            <tr>
                <td>Diedit</td>
                <td>:</td>
                <td class="ps-2 font-price fw-light fsz-10 text-secondary">@if($berita->created_at !== $berita->updated_at) {{ $berita->updated_at }} @else Belum pernah diubah. @endif</td>
            </tr>
        </table>
        <a href="{{ url('admin-edit-berita/' . $berita->berita_id) }}" class="btn btn-sm p-0 px-2 fsz-10 btn-warning" style="width:100px;"><i class="fas fa-pencil"></i> edit</a>
    ...
    <div class="m-0 p-0">
        <div class="border-clr3 p-3 overflow-x-hidden w-100 pt-4" id="kontenID">
            <h1 class="mb-4">{{ $berita->berita_judul_ID }}</h1>
            {!! $berita->berita_isi_ID !!}
        </div>
        <div class="border-clr3 p-3 overflow-x-hidden w-100 pt-4 d-none" id="kontenEN">
            <h1 class="mb-4">{{ $berita->berita_judul_EN }}</h1>
            {!! $berita->berita_isi_EN !!}
        </div>
    </div>
</div>
...
<div class="modal fade" id="modalImage" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered modal-dialog-scrollable">
        ...
        <div class="modal-header">
            <div class="my-2">
                <h1 class="modal-title fs-5 lh-1 mb-1" id="">{{ $berita->berita_judul_ID }}</h1>
                <p class="text-secondary m-0 lh-1">{{ $berita->berita_judul_EN }}</p>
            </div>
...
<div class="modal fade" id="modalDelete" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered modal-dialog-scrollable">
        ...
        <div class="modal-body">
            <p class="m-0">Apakah Anda yakin ingin menghapus berita ini?</p>
        </div>
        <div class="modal-footer">
            <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Close</button>
            <a href="{{ url('admin-hapus-berita/' . $berita->berita_id) }}" class="btn btn-danger">Hapus</a>
        </div>
        </div>
    </div>
</div>
