<div class="text-clr3 m-0 p-0 col-xl-8">
    ...
    <hr>
    <form action="{{ url('admin-update-matkul') }}" method="post" enctype="multipart/form-data">
        @csrf
        <input type="hidden" name="mk_id" value="{{ $mk->mk_id }}">
        <div class="row m-0 p-0 mb-3">
            <div class="col-md-6 m-0 p-0 pe-0 pe-md-1">
                <label for="mk_id">Kode MK</label>
                <div class="he-25">{{ $mk->mk_id }}</div>
            </div>
        </div>
        <div class="row m-0 p-0 mb-3">
            <div class="col-md-6 m-0 p-0 pe-0 pe-md-1">
                ...
                value="{{ old('mk_mk_ID', $mk->mk_mk_ID) }}">
            </div>
            <div class="col-md-6 m-0 p-0 ps-0 ps-md-1">
                ...
                value="{{ old('mk_mk_EN', $mk->mk_mk_EN) }}">
            </div>
            <div class="col-md-6 m-0 p-0 pe-0 pe-md-1">
                @error('mk_mk_ID')
        ...
        <div class="row m-0 p-0">
            <div class="col-md-6 m-0 p-0">
                ...
                <div class="mb-3">
                    ...
                    <select class="cursor-pointer bg-transparent he-28 border-clr3 ms-3" aria-label="mk_semester" name="mk_semester">
                        ...
                        <option value="1" {{ old('mk_semester', $mk->mk_semester) == '1' ? 'selected' : '' }}>1</option>
                        <option value="2" {{ old('mk_semester', $mk->mk_semester) == '2' ? 'selected' : '' }}>2</option>
                        <option value="3" {{ old('mk_semester', $mk->mk_semester) == '3' ? 'selected' : '' }}>3</option>
                        <option value="4" {{ old('mk_semester', $mk->mk_semester) == '4' ? 'selected' : '' }}>4</option>
                        <option value="5" {{ old('mk_semester', $mk->mk_semester) == '5' ? 'selected' : '' }}>5</option>
                        <option value="6" {{ old('mk_semester', $mk->mk_semester) == '6' ? 'selected' : '' }}>6</option>
                        <option value="7" {{ old('mk_semester', $mk->mk_semester) == '7' ? 'selected' : '' }}>7</option>
                        <option value="8" {{ old('mk_semester', $mk->mk_semester) == '8' ? 'selected' : '' }}>8</option>
                    </select>
                    @error('mk_semester')
                    <div class="ms-1 fsz-10 mt-1 text-clr1"><i class="fa-solid fa-circle-exclamation"></i> {{ $message }}</div>
                    @enderror
            ...
            <div class="col-md-6 m-0 p-0">
                ...
                <div class="border-clr3 p-2 bg-light mt-2">
                    @include('admin/matkul-edit-kurikulum')
                </div>
            </div>
        </div>
        <hr class="my-4">
        <div class="row m-0 p-0">
            <div class="col-lg-6 m-0 p-0 pe-lg-1 mb-3">
                ...
                <div class="ms-1 fsz-10 text-clr1"><i class="fa-solid fa-circle-exclamation"></i> {{ $message }}</div>
                @enderror
                <textarea name="mk_deskripsi_ID" id="mk_deskripsi_ID" class="border-clr3 w-100 p-2 bg-transparent" style="height:120px;" placeholder="...">{{ old('mk_deskripsi_ID', $mk->mk_deskripsi_ID) }}</textarea>
            </div>
            <div class="col-lg-6 m-0 p-0 ps-lg-1 mb-3">
                ...
                <div class="ms-1 fsz-10 text-clr1"><i class="fa-solid fa-circle-exclamation"></i> {{ $message }}</div>
                @enderror
                <textarea name="mk_deskripsi_EN" id="mk_deskripsi_EN" class="border-clr3 w-100 p-2 bg-transparent" style="height:120px;" placeholder="...">{{ old('mk_deskripsi_EN', $mk->mk_deskripsi_EN) }}</textarea>
            </div>
        </div>
        ...
        <form class="modal-content" action="{{ url('admin-session-kurikulum') }}" method="post">
            ...
            <div class="modal-body">
                ...
                @if(session()->has('session_kurikulum'))
                    ...
                    <?php $i++ ?>
                    @endforeach
                @elseif($mk->kurikulum->isNotEmpty())
                    @foreach($mk->kurikulum as $x)
                    <div class="my-2 d-flex">
                        <input type="checkbox" name="kurikulum_{{ $i }}" id="kurikulum_{{ $i }}" value="{{ $x['kurikulum_id'] }}" {{ $x->kurikulum_selected ? 'checked' : '' }}>
                        <div class="ms-2">
                            <label for="kurikulum_{{ $i }}">
                                <h5 class="m-0 lh-1">{{ $x->kurikulum_judul_ID }}</h5>
                                <p class="m-0 lh-1">{{ $x->kurikulum_judul_EN }}</p>
