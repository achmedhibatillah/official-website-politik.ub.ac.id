<div class="text-clr1 m-0 p-0">
    <h4>Edit Kurikulum</h4>
    <hr>
    <div class="alert alert-danger alert-dismissible fade show text-center d-none" role="alert" id="alertUbah">
        Belum ada data yang diubah.
        <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
    </div>
    <form action="{{ url('admin-update-kurikulum') }}" method="post" enctype="multipart/form-data">
        @csrf
        <input type="hidden" name="kurikulum_id" value="{{ $kurikulum->kurikulum_id }}">
        <div class="mb-3">
            ...
            value="{{ old('kurikulum_judul_ID', $kurikulum->kurikulum_judul_ID) }}">
        ...
        <div class="mb-3">
            ...
            value="{{ old('kurikulum_judul_EN', $kurikulum->kurikulum_judul_EN) }}">
        ...
        <div class="mb-3">
            <div class="row m-0 p-0">
                <div class="col-6 m-0 p-0 pe-1">
                    ...
                    value="{{ old('kurikulum_mulai', $kurikulum->kurikulum_mulai) }}">
                </div>
                <div class="col-6 m-0 p-0 ps-1 position-relative">
                    <label for="kurikulum_silabus">Ubah file silabus (PDF) <a href="{{ url($kurikulum->kurikulum_silabus) }}" target="blank" class="td-none">Lihat file lama</a></label>
        ...
        <div class="mb-3">
            ...
            <div class="ms-1 fsz-10 mt-1"><i class="fa-solid fa-circle-exclamation"></i> {{ $message }}</div>
            @enderror
            <textarea id="kurikulum_deskripsi_ID" name="kurikulum_deskripsi_ID" class="border-clr1">{{ old('kurikulum_deskripsi_ID', $kurikulum->kurikulum_deskripsi_ID) }}</textarea>
        ...
        <div class="mb-3">
            ...
            <div class="ms-1 fsz-10 mt-1"><i class="fa-solid fa-circle-exclamation"></i> {{ $message }}</div>
            @enderror
            <textarea id="kurikulum_deskripsi_EN" name="kurikulum_deskripsi_EN" class="border-clr1">{{ old('kurikulum_deskripsi_EN', $kurikulum->kurikulum_deskripsi_EN) }}</textarea>
            <script src="https://cdn.ckeditor.com/4.18.0/standard/ckeditor.js"></script>
            <script>
                CKEDITOR.replace('kurikulum_deskripsi_EN');
            </script>
        </div>
        <button class="btn btn-outline-clr1 btn-sm mt-2" type="submit" id="btnSubmit">Simpan</button>
    </form>
</div>
